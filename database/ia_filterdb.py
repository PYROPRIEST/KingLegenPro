...
async def get_bad_files(query, file_type=None, max_results=100, offset=0):
    return await get_search_results(query, file_type=file_type, max_results=max_results, offset=offset)

async def get_file_details(file_id_query):
    cursor = Media.find({'file_id': file_id_query})
